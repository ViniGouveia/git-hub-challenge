...
interface ServiceApi {
    ...
    @GET("users/{username}")
    suspend fun fetchUserInfo(@Path("username") username: String): Response<UserInfoResponse>

    @GET("users/{username}/repos?per_page=100")
    suspend fun fetchUserRepositories(@Path("username") username: String): Response<List<RepositoryResponse>>
}
